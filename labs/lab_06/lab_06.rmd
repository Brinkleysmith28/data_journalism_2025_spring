...
party_registrations <- read_csv("data/new_registrations_by_party.csv")
party_registrations
...
removals <- read_csv("data/removals_by_party.csv")
removals
...
```{r}
# Changes
changes <- read_csv("data/changes_in_registration.csv")
changes
...
current_registrations <- read_csv("data/current_registrations_by_county.csv")
...
```{r}
# Party registrations
party_registrations_2022 <- read_csv("data/2022_new_registrations_by_party.csv")
...
```{r}
# Removals
removals_2022 <- read_csv("data/2022_removals_by_party.csv")
removals_2022
```

```{r}
# Changes
changes_2022 <- read_csv("data/2022_changes_in_registration.csv")
changes_2022
```

```{r}
# Current registrations
current_registrations_2022 <-read_csv("data/2022_current_registrations_by_county.csv")
...
A1. While the democrats saw a 2% change, the republican party saw a smaller decrease in registered voters, less than one percent. The lede of the story and the most interesting thing was regarding unaffiliated voters. There was over an 8% increase.
...
merged_data <- merge(voter_data_2024, voter_data_2022, by = "party")
...
A2. The county with the biggest percentage increase in unaffiliated voters between 2022 and 2024 was Baltimore County with an 83% increase. In 2020, Baltimore County voted for Biden and in 2022 the county voted for Wes Moore. With -11%, Baltimore City had the lowest percentage change.
...
voter_data_unaf <- voter_data |>
  arrange(desc(percentage_change))

print(voter_data_unaf)
```
...
A3. No, it did not match. There was a 21% increase in the percentage change for republicans which is fairly significant. The further exploration of methods like where an individual registered could be worthy of exploration. 
...
merged_data <- merge(new_registrants_2024, new_registrants_2022, by = "party")
...
A4. Q:What are the percentage changes in the non-main parties and what trends stick out? A: In the green party, there's been a significant decrease of members. If this trend were to continue, it could diminish the party completely. 
...
total_voters_2022 <- sum(voter_data_2022$total_registered)
voter_data_2022$percentage_2022 <- (voter_data_2022$total_registered / total_registered_2022) * 100
...
A5. Why is it that more individuals are choosing to identify as unaffiliated? We saw in the first question that there was an 8% increase in unaffiliated voters. To conduct this story we'd need to complete a lot of interviews but it could be Interesting to see what may be the cause of the shift in way voters are registering for political parties.
