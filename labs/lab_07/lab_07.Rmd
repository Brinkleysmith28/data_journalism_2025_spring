...
```{r echo=FALSE}
census_api_key("725ab0e0f0a2f0fedfbc3490050e3a6ed61ca741", overwrite=TRUE)
```
...
**A1: Prince George's County has the lowest median age with 38.2 years old.**

```{r}

md <- get_acs(geography = "county", variables = c(med_age = "B01002_001"), state = "MD", year = 2022)
...
**A2:  Talbot County and Worcester County were the only two counties with a median age of at least 50. Talbot County had 51.4 while Worcester County had 50.6 **
...
**A3: There was 11,871 people who voted for different parties. 95,221 voted for the same party. Of all voters, 11.1% split their ticket. 89,916 people voted the same party in all five races. 83.8% of all voters voted for all five.**
...
**A4: The largest number was 21,702.**
...
**A5: 21702 is the top zipcode which is located in Frederick, Maryland. Three out of the top five are located uin Frederick, Maryland while the other two are from Walkersville and Mount Airy. The median age for all the top zips seem lower than the zips with the fewest voters. For example, the median age for the top age is 38.1 comapred to 47.3 for the lowest.  **

```{r}
md_age <- get_acs(geography = "zcta", variables = c(median_age = "B01002_001"), year = 2022)
md_age <- md_age|>
