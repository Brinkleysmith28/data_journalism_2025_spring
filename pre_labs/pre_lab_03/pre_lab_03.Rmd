...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer: 1,640 rows but limited 10 rows. The readr library exacted true or false. It didn't read the value correctly.*
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: No errors were shown.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer: District and mail are numbers. Precinct is under characters.**
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer: There are 402,345 where mail is NA. This could mean that not all mail-in voting was all reported on.**:
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer: The deceased reason would be worth exploring because it should be know if these voters were removed from active registration or not. I also think the legacy data should be explored more since the category itself and its description is vague.**
...
**Answer: Column names could have spaces in them. Inconsistent capitalization across multiple columns. The "zip" field has some five and some nine digit numbers, some even with spaces.**
...
**Answer: There is now a "X" in front of the "1".**
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer: It says my error was in 'summarise()' caused by error in 'sum', invalid 'type'. Sum only works on numbers.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer: There are two duplicates and the name is Derrick Hamilton.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer: There are 13 rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer: Takes the nine digits and just shortens it to the last five digits only.**
...
**Answer: There are now 13 misspellings.**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer: There are still 13 misspellings.**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer: There's still 13 misspellings.**
