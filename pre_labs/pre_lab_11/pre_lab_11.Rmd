...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer: The jurisdiction column appears to be labeled correctly. However, the next two columns aren't right. The columns names are long. They need to be renamed. Also, the columns are being stored as chr and not as a dbl bc of the commas in the number.**
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer: The numbers correlate with who is in each column. For example, Harris is 2 because she is the second column. Trump is 3 because he is the third column and so on and so forth. The numbers refer to the position of the columns.**
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer: This could suggest that there are multiple <ul> lists on the page. There are also repeated navigation links.**
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer: The output shows the navigation, committee names and footer links. On the list below you will see that 21-46 are the only ones that contain the committee names. There also appears to be extra space.**
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer: The output is a dataframe called committees_with_urls where there are two columns. It organizes committee name and their corresponding URL.**
