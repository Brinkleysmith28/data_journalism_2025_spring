...
**Answer: https://datawrapper.dwcdn.net/T6pl5/1/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
```{r}
# Load the election results CSV
md_gov_county <- read_csv("data/md_gov_county.csv")

# Add columns for total votes and percentage of votes for Wes Moore
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer: The change is that Maryland is the only state that you can see now and all you see is the state and the multiple counties, each different colors.** Describe the changes from the previous map
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Answer: Now the counties are labeled when you hover over them and tell the perectnage of each county.** Describe the changes from the previous map
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer:https://datawrapper.dwcdn.net/DKQ82/1/** Put the URL of your map here.
