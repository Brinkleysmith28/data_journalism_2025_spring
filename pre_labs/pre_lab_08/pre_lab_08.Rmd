...
**Answer: The mutate is creating a new column within the md_voters dataframe that shows the difference in the change in unaffiliated voters by subtracting the 2024 from the 2022 numbers. The arrange with descending allows the biggest number to be displayed at the top. Select allows you to pick the specific columns you want to keep and ignores the rest of the columns. Head(10) is saying that only the top 10 rows will be displayed from the sorted data. The maryland_top_counties is the dataframe that is displaying the top 10 results. **
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: All I see is a blank canvas.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I see a bar chat which grey colored bars. However, the chart is hard to read with the bars in all different orders the county names overlapping one another.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: What has changed from the last codeblock was the bars are now ordered correctly which makes it easier to read.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: Since the last codeblock, the bars are now horizontal and in order accordingly. Also, the county names aren't overlapping one another, making the chart easy to read. **
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: The biggest difference I notices was the background color of the chart. The chart above has a grey background while the new chart below has a white background. This is because we added the theme_minimal to the chart below.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: The theme economist included a bold axis line, thicker gridlines, and a light blue background contrasted by grey colored bars. But the reorder county axis is too close to the beginning of the county names.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer" While the theme remained the same, more labels have now appeared on the chat. There is now a title at the top, a caption with the source and county and net chnage on the axis.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: The group_by is for the date and that day. The summarise shows the number of total contributions made on that date. The md_winred_by_date is where new dataframe where the results were stored to.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I see a line chart that ranges from April to July.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: I still see a line chart but now the x-axis is customized to show a tick every week.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: Now the ticks that are on the x-axis are titlted at an angle so they aren't overlapping and aren't crammed next to one another anymore.**
