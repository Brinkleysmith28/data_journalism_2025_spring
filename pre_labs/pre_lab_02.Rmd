...
**Answer:The code is taking the election day votes and total votes and dividing it to get the percentage of votes for a specific candidate that were cast on election day.**
...
**Answer: The numbers for percent_election_day are now in percentage format since multiplying by 100 at the end. The results are also now arranged which displays the highest percentage,100%, at the top making it easier to see which candidates received the highest proportion of votes. **
...
**Answer: Depending on which way you want to look at the data this could be more interesting. If you want to analyze the candidate who received the least amount of votes on election day then ascending order would work better. This would allow you to see how many votes a candidate received and see if that candidates voters are more likely to vote on election day or not.**
...
**Answer: str_to_lower converts to lower case and str_to_title converts to title case, where only the first letter of each word is capitalized.**
...
**Answer: The mutate statement is now creating a column for amount. From there, the 'case_when' function is categorizing different range amounts.**
...
**Answer: It's important because it allowed us to store and keep the filtered and simplified version of the dataset.**
...
**Answer: The change is that it in now a <date> column and not a <chr> column. It is also now formatted YYYY-MM-DD.**
...
**Answer: The data is showing the total campaign expenditures for each month. This can allow for analysis of campagin spending over the year.**
