...
**Answer:There are 131395 observations and 24 variables.**
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **Answer:last_name is <chr> and amount is <dbl>. The last_name column could represent the last names of those individuals in a data set. The amount column could represent how much money those individuals contributed.**
...
**Answer: The code is grouping the data set by specific dates and the amount of contributions made on that date. Head() shows data from the first few rows that is ungrouped data. However, this code makes it easier to analyze specific data.**
...
**Answer: The thing that changed was how the data was arranged. This code is specific and says arrange desc compared to the last codeblock that wasn't in any particular order. Now the data is easier to analyze because the highest numbers of contributors will appear first.**
...
**Answer: Now the data is grouped by memo_text and amount. This allows an individual to see the specific organization, committee, etc and the amount and count contributions they're recieving.**
...
**Answer: This codeblock allows individuals to see the organization or committees that received the most contributions.**
...
**Answer: The new codeblock now adds mean_amount and median_amount. Adding this allows an individual to see average and typical contribution was. One of the top results from Maryland include HOGAN FOR MARYLAND INC. **
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Answer: The min_amount and max_amount have now been added to the codeblock. This allows a user to see what the smallest and largest contribution made from each city was. **
