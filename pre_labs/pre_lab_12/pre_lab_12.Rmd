...
**Task** Run the following code and describe the dataframe it outputs. **Answer: The dataframe is called releases. There are four columns, date, title, url and text.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: The code returns word dog and cat and returned a comma. The list shows the unique version.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: There is a dataframe called unique words now. With one column called word, each row has a word in it. All words are lowercase and no punctuation.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer: While the concept of it is neat, it isn't very interesting since it is common words that are taking up the top 25 spot.**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer: Yes it is more interesting than before. Instead of seeing common repeated words you now see topic words that could give someone an idea on what is being focused on.**
...
**Task** Run the following code and describe the results. **Answer: There are two list one from 2022 and one from 2024 that display the senators most frequently used words. You can also see the percentage of how much it was used that year. Health and Baltimore being at the top of each list.**
...
**Task** Run the following code and describe the results. **Answer: Now we see the top 10 two-word phrases that the senator used from 2022. Amount included was used the most followed by health care.**
...
**Task** Run the following code and describe the results. **Answer: Answer: Now we see the top 10 two-word phrases that the senator used from 2024. Health care was used the most followed by amount awareded.**
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer: Lemmatization seems to be more useful for the press release data.**
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer: The lemma is more useful. You are able to get a clearer view of key themes.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: The results show the most common positive and negative words from 2022 and 2024 based on the bing lexicon. Some results may seem inaccurate because certain words can be used positively even if they’re labeled as negative.**
