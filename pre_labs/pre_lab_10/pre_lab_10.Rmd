...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer: The data set for election precincts in Prince George's County, Maryland. At the bottom it also say Multipolygon.There are 10 columns which are PRECINCT_ID, PRECINCT_NAME, CONGRESS, LEGIS, COUNCIL, SCHOOL, POLLING_ID, Shape_Length, Shape_Area, and Shape.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer: 356 rows and 10 variables. The columns are the same as listed above. Also multipolygon is in use.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code brought the data to life. the precinct boundaries are outlined in gray. Theme_minimal adds a simple and minimalist map style. There are also latitude and longitude grid lines and labels that correspond. This is also the full shape of Prince George's Countu with its precinct boundaries.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code loads the data set of all U.S. counties using the counties function with the help of tigris. It retrieved the data for the year of 2022 it says. We can see that there is 3,235 rows and 18 columns. Below the columns and their names are listed. We can also see at the bottom that Multipolygon is listed again.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code filtered and made a new md_counties that will now only display Maryland counties. The rows drop down to 24 now. All the original names remain the same.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code created a map of Maryland. Each county is clearly outlined and filled in with a light grey. There are also latitude and longitude lines that are labeled. Once again, there is a theme_minimal that makes the graph easy to read and not distracting.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: I see Prince George's county outlined and each of its precinct boundaries. There is also a stronger boarder compared to the one that we loaded a few above. There appears to be more depth in this map. *
...
**Task** Run the following code. Describe the output in the space below. **Answer: This loads voter turnout for Prince George's county. There are 11,886 rows and 10 columns( 5 CHR and 5 DBL) We can also see the column names listed below.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The performs a left join. Now the pg_precincts and pg_turnout were put together. Now there is a new dataframe that is called pg_precincts_with_turnout.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now there is a map of Prince George's County but this time each precinct is filled with a shade of blue according to the scale of total eligible to the right. The dark blue represents fewer votes while the lighter colors represent higher numbers of eligible voters. The map still contains all the other map elements that has been included in the other previous maps.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code runs the eligible voters file. There are 24 rows and 9 columns where each row marks a Maryland county. The columns show the party affiliations and the number of registered/eligible voters for each.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now there is a date frame named md_county_population and GEOIDS and the variables where we have the populations.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code gets data from 2018-2022. The result is a spatial dataset where each row represents a county and includes its name, total population, and a multipolygon geometry. This prepares the data for both analysis and visualization, allowing us to join it with voter registration data and display trends on a map. **
...
**Task** Run the following code. Describe the output in the space below. **Answer: The output below shows mismatched county names and their formatting**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code cleans the population data so it can be joined with the voter registration data. It renames the NAME column to County, removes the extra text like "County" and ", Maryland", trims any extra white space, and converts the names to title case. It also renames the estimate column to population and keeps only the County, population, and geometry columns. The result is a clean, spatially-enabled table with 24 Maryland counties that now matches the format of the voter data and is ready for a join.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code joins the cleaned population data with the voter registration data by the County column. The result, md_voters_per_10k, is a spatial dataset that now includes each county’s population, voter registration breakdown, and geometry for mapping. However you can see that some rows have NA listed and that is where the join didn't match.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This code fixes mismatched county names that were preventing a successful join with the voter data. It uses if_else to: Change "St. Mary's" to "Saint Mary's" and Change "Baltimore" to "Baltimore County"**
...
**Task** Run the following code. Describe the output in the space below. **Answer:This code joins the voter registration data with the county population data, then calculates a new column called voters_per_10k, which represents the number of registered voters per 10,000 residents in each county. The results are sorted from highest to lowest.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: There is now a chorpleth map of Maryland counties. Each county is a shade based on its number of registered voters per 10,000 residents. The light blue represents the higher numbers while the darker blue represents the lower numbers.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This updated map uses the "magma" color scale to show the number of voters per 10,000 people in each Maryland county. The colors make it easier to disinguish what numbers correspond with the correct numbers.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer: This map visualizes voter registration rates per 10,000 people across Maryland counties using the magma color scale with a log transformation. The log scale improves visual contrast by stretching differences among mid-range values, making it easier to compare counties that would otherwise look similar on a linear scale.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer: Maybe you can help me understand why this one gave me an error code and I was unable to see a map below. Thank you.**
